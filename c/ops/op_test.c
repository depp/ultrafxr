...
static const struct func_info kFuncs[] = {
    F(exp2_2, exp2_err, 2.989197f), //
    F(exp2_3, exp2_err, 0.129604f), //
    F(exp2_4, exp2_err, 0.004721f), //
    F(exp2_5, exp2_err, 0.000572f), //
    F(exp2_6, exp2_err, 0.000286f), //
    F(tri, tri_err, 2.0f),          //
};
#undef F
...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(kFuncs); i++) {
        ...
        if (error > kFuncs[i].error * kErrorMargin) {
            puts("****FAIL****");
            success = false;
        }
        putc('\n', stdout);
